...
DOMAIN_STOP_WORDS = [
    # Bruit structurel identifié dans vos tests
...
def run_sequential_clustering(resumes, concepts):
    ...
    for c_idx, c in enumerate(concepts):
        ...
        # Tolère les pluriels (-s, -es) et les participes/passés (-d, -ed, -ing)
    ...
    for r_id, members in root_clusters.items():
        ...
        if len(members) > 15:
            # Sous-clustering pour les piliers trop denses
            sub_matrix = filtered_matrix[root_indices[r_id]]
            hac_s = AgglomerativeClustering(n_clusters=4, linkage='average', metric='cosine')
            sub_lbls = hac_s.fit_predict(sub_matrix)
            sub_map = defaultdict(list)
            for i, sl in enumerate(sub_lbls): sub_map[sl].append(members[i])
    ...
    # Traitement LDA dépollué pour le
...
# Dictionnaire d'exceptions pour forcer le maintien du terme métier (en minuscules)
IT_VOCAB_EXCEPTIONS = {
    "data": "data",
    "cyber": "cyber",
    "ai": "ai",
...
def clean_and_normalize(concepts: set[str]) -> list[str]:
    ...
    for c in tqdm(concepts):
        ...
        for t in doc:
            ...
            # Interception : si le mot est protégé, on bloque la lemmatisation
            if token_lower in IT_VOCAB_EXCEPTIONS:
                normalized_tokens.append(IT_VOCAB_EXCEPTIONS[token_lower])
            else:
                # Sinon, on applique le lemme standard (ex: "Models" -> "Model")
                normalized_tokens.append(t.lemma_)
        
        # Reconstruction et normalisation de la casse (Title Case)
        normalized_concept = " ".join(normalized_tokens).title()
        
        # Filtrage strict de la longueur (élimine les acronymes isolés suspects et les phrases)
